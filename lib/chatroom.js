...
export class Chatroom {
  ...
  groupCall(stream) {
    ...
    for(const conn of this.members.values()) {
      ...
      call.on('close', () => this._removeFromCallGroup(call))
    }
  ...
  _addToCallGroup(call, callStream) {
    ...
    View.showCall(document.getElementById('chat-output'), callGroup)
    if (memeberCount === 0) {
  ...
  _removeFromCallGroup(call) {
    document.getElementById(call.peer + '-call').remove()
  }
}