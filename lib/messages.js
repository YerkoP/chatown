export function showInfo(msg) {
  ...
  if (typeof msg === 'object') {
    p.innerHTML = msg.text.replace('{cta}', `<a id=${ctaId} class="cta cta-copy underline cursor-pointer">${msg.cta}</a>`)
...
export function showMsg(msg, peer) {
  ...
  span.classList.add('px-4', 'py-1', 'rounded-lg', fromMyself ? 'bg-slate-950' : 'bg-slate-800')
  p.appendChild(span)
  const chatOutput = document.getElementById('chat-output')
